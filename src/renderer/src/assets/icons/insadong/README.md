...
The 16 home-grid tiles are now `.svg`; `kdrama` and `restroom` are still
`.png`/`.svg`. Names below are listed with the legacy `.png`
extension but either extension resolves.
...
- `ai-search.png`   — '인사' 모하지 (AI검색) — the robot tile (wide)
- `market.png`      — 위드마켓
- `events.png`      — 인사동 이벤트
- `eat.png`         — '인사' 뭐먹지
- `shop.png`        — '인사' 뭐사지
