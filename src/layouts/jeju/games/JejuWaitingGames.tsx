 ...
 *
 * Deliberately long — a full minute of nothing. It is the only way the photo
 * ever takes the screen without a tap, and it exists for a kiosk somebody has
 * walked away from, not for a visitor thinking. Armed only once the photo is
 * ready, because before that there is nothing to release to.
 */
...
export function JejuWaitingGames({ rounds, aiReady, onFinish, onHome }: Props): JSX.Element {
  ...
  //
  // Asked ONCE, the moment the photo lands, and only over a game: on the menu
  // the status line itself turns into the 사진 보기 button, which is the same
  // question already. Never an automatic hand-over — see PhotoReadyPrompt.
   ...
   * the photo lands, "finished" means "back to the menu", where the other game
   * is and an honest status line. After it lands this only runs from the
   * visitor's own 사진 보기 tap — 틀린그림찾기 no longer hands over by itself.
   */
