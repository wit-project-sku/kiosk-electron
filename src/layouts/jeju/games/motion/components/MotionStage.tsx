...
// for the customer display — see `.onDisplay` there.
...
interface Props {
  ...
  phase: MotionPhase;
  /** Live score, pushed up to Monitor 1's remote and shown top-right. */
  score: number;
  /** Best score this session, shown under the live one once there is one. */
  best?: number;
  ...
  /** Extra HUD content positioned on the artboard by the game, if any. */
  hud?: ReactNode;
  /** The play field. */
  children: ReactNode;
  /** The score card, or null while playing. */
  result?: ReactNode;
  /** A one-line notice at the foot of the field — e.g. "your photo is ready". */
  notice?: string | null;
}
...
export function MotionStage({
  ...
  return (
    <div className={`${styles.displayRoot} ${shared.onDisplay}`}>
      {/* One header row: what the game is on the left, how it is going on the
          right. It replaces a centred title, a subtitle, a gradient rule and a
          pair of glass score pills that all competed for the same glance. */}
      <header className={styles.displayHeader}>
        ...
        <div className={styles.displayScore}>
          <span className={styles.displayScoreLabel}>{pick(TEXT.score, lang)}</span>
          <span className={styles.displayScoreValue}>{score}</span>
          {best > 0 && (
            <span className={styles.displayBest}>
      ...
      {/* Informational only — nothing on this screen is pressable. Hidden at the
          hand gate, which already fills the screen with one instruction. */}
      {notice && !calibrating && (
      ...
      {phase === 'playing' && <MotionCoach status={tracking.status} source={tracking.source} />}
