...
/**
 * Inference pacing: once per NEW camera frame, but never more often than this.
 *
 * ══ WHY THIS STOPPED BEING A FIXED 50ms TIMER ═════════════════════════
 * 20/s was chosen for a body steering a basket, where position is smoothed and
 * a person cannot change direction inside 50ms. 제주 달리기 is played with hand
 * SHAPES, and a shape change is a discrete event the visitor expects to see
 * answered at once. A free-running 50ms timer, unsynchronised with the camera,
 * added up to a whole frame of waiting before a new frame was even looked at,
 * and inferred on repeated frames in between. Visitors described it as slow.
 *
 * Now each pass is scheduled by `requestVideoFrameCallback`, so inference
 * starts the moment a frame arrives — about 30/s on the 제주 cameras — with this
 * floor so a 60fps camera does not double the cost. {@link STALL_TICK_MS} keeps
 * the loop (and its liveness diagnostics) running when no frames arrive.
 */
const MIN_INFERENCE_INTERVAL_MS = 30;
/** Fallback tick when the camera delivers no frame — see MIN_INFERENCE_INTERVAL_MS. */
const STALL_TICK_MS = 120;
...
export function useMotionTracking({ enabled }: Options): MotionTracking {
  ...
  useEffect(() => {
    ...
    const tick = (): void => {
      ...
      scheduleNext(started);
    };

    /**
     * Run the next pass on the next camera frame, or after {@link STALL_TICK_MS}
     * if none arrives. Whichever comes first wins; the other is a no-op because
     * `pending` has moved on. A cancelled hook leaves at most one dangling
     * callback, which `tick` ignores.
     */
    let pending = 0;
    const scheduleNext = (started: number): void => {
      const token = ++pending;
      const run = (): void => {
        if (token !== pending || cancelled) return;
        const wait = MIN_INFERENCE_INTERVAL_MS - (performance.now() - started);
        if (wait > 1) {
          loopTimer = setTimeout(run, wait);
          return;
        }
        pending += 1;
        tick();
      };
      loopTimer = setTimeout(run, STALL_TICK_MS);
      if (videoElement && 'requestVideoFrameCallback' in videoElement) {
        videoElement.requestVideoFrameCallback(() => run());
      }
    };
