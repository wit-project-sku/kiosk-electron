...
import { RunControl as RunControlDetector, type ControlShape } from './runControl';
 ...
 *
 *   jump     "make a V-sign ✌️"  → waits for a real jump
 *   jump-ok  "잘했어요!"          → a beat of praise
 *   duck     "make a fist ✊"    → waits for a real duck, held briefly
 *   duck-ok  "잘했어요!"
...
export interface RunControl {
  /**
   * BODY fallback only: height against the baseline in THRESHOLD units, -1 the
   * jump line and +1 the duck line. Null for a hand, which plays by shape.
   */
  level: number | null;
  /** The shape the hand is steadily holding — lights the meter's badge for it. */
...
export function useJejuRun(player: React.RefObject<PlayerTrackingState>): JejuRunApi {
  ...
  // Read on every animation frame, acting only when the tracker has produced a
  // NEW reading (or has lost the hand, which is time-based). This used to be a
  // fixed 50ms interval, and because it was not in step with the tracker a
  // fresh hand shape could sit unread for most of a frame before the runner
  // reacted — one of the two delays visitors felt as "slow".
  useEffect(() => {
    ...
    const step = (): void => {
      ...
      const seen = state?.detected ? state.lastSeenAt : null;
      if (seen !== null && seen === lastSeen) return;
      lastSeen = seen ?? -1;
      const sample =
      ...
      // After the duck state, so a fist switched straight to ✌️ stands her up
      // and jumps on the same tick.
      if (out.jump) {
        engine.jump();
        sfx.tap();
