...
/**
 * Left rail shared across 화성휴게소 screens — home, back and the ♿ 베리어프리
 * toggle, 85×85 on a 120 step, at the 제주 rail position (x50, y2043).
 */
export function HwaseongLeftNav({ onHome, onBack, low = false }: HwaseongLeftNavProps): JSX.Element {
  ...
  return (
    <div className={`${styles.leftNav} ${low ? styles.leftNavLow : ''}`}>
      <button type="button" className={styles.leftNavBtn} onClick={onHome} aria-label="홈으로">
        {hwaseongIconUrl('home-btn') && <img src={hwaseongIconUrl('home-btn')} alt="" draggable={false} />}
      </button>
      <button type="button" className={styles.leftNavBtn} onClick={onBack ?? onHome} aria-label="뒤로">
        {hwaseongIconUrl('back-arrow') && <img src={hwaseongIconUrl('back-arrow')} alt="" draggable={false} />}
      </button>
      <button
        type="button"
      ...
      >
        {/* Wheelchair glyph (Material "accessible"), drawn in currentColor so it
            takes the 화성휴게소 brand colour from the stylesheet. */}
